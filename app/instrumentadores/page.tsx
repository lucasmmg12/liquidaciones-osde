...
import { exportLiquidacionCompleta, exportDetalleToExcel, exportResumenToExcel, exportFaltantesToExcel } from '@/lib/excel-exporter';
...
export default function InstrumentadoresPage() {
  ...
  const handleExportFaltantes = () => {
    try {
      const filename = `codigos-faltantes-${mes}-${anio}.xlsx`;
      exportFaltantesToExcel(faltantes, filename);
      toast({
        title: 'Exportación exitosa',
        description: `Listado de faltantes descargado: ${filename}`,
      });
    } catch (error) {
      console.error('Error exporting faltantes:', error);
      toast({
        title: 'Error',
        description: 'No se pudo exportar el listado de faltantes',
        variant: 'destructive',
      });
    }
  };
                      ...
                      {faltantes.length > 0 ? (
                        <>
                          ...
                          <div className="flex gap-2 mb-4">
                            ...
                            <Button onClick={handleExportFaltantes} variant="outline">
