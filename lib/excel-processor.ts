...
function extractCodigoYProcedimiento(valor: string): { codigo: string; procedimiento: string } {
  if (valor.includes('-')) {
    // Formato: "200138-FIBRO" -> código: "200138", procedimiento: "FIBRO"
    const partes = valor.split('-');
    return {
      codigo: partes[0].trim(),
      procedimiento: partes.slice(1).join('-').trim()
    };
  } else {
    // Si no tiene guión, asumir que es solo el código o el código completo
    return {
      codigo: valor.trim(),
